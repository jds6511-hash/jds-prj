...
```
DROP 프레임 중 novelty가 0에 가까운 것은 없다 (최소 5.7761).
즉 빠지는 프레임은 어느 것도 양쪽 이웃의 near-duplicate가 아니다.
```

**이것을 semantic loss라고 부르지 않는다.** 픽셀 변화량은 의미가 아니다 — 카메라
...
사전등록돼 있다. 최대/최소 비는 1411.2 / 816.3 ≈ 1.73으로, 변화가 특정 구간에
몰려 있지 않다.

## 4. 계측 사고 1건 — 기록

첫 Stage 1 실행 산출물은 **폐기했다.** `frame_size`가 512×288이 아니라 384×288로
기록됐다.
...
영향   Stage 1 재실행. 서버에서 돌린 C01·A0·A1·B는 별도 사본이라 영향 없다.
       참고로 오염 실행에서도 창 선택은 W11·W02·W05로 같았으나, 보고 수치는
       모두 정상 실행분이다.
```
