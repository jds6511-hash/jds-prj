...
```
A  chunk 길이를 줄인다                    이번 사건에서 금지된 그 축이다.
                                        새 사전등록으로만 가능하다
B  프레임 표집을 줄인다                    fps 0.5 → 0.25, 또는 512×288 → 384×216
                                        (32의 배수 유지 필요)
C  attention 구현을 바꾼다                 flash-attn 2 설치 여부 미확인.
                                        prefill 중간 텐서를 줄이는 축이다
D  가중치를 양자화한다                     8bit·4bit. 17.3GB → 절반 이하.
                                        정확도 영향이 있어 별도 판단이 필요하다
E  더 큰 VRAM                             랩실에 4090 24GB 외 선택지가 있는지 미확인
```

**속도가 아니라 용량이 막았다는 점이 중요하다.** 정확도 손실을 감수하는 축(D)과
감수하지 않는 축(B·C)은 성격이 다르므로 섞어 고르면 안 된다.
