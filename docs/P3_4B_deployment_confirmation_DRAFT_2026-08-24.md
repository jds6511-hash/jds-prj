...
```
1  ICC = 0이므로 **총 행 수가 m과 거의 무관하다.** m 선택은 라벨 총량 문제가 아니라
   수집 비용(영상 편수)과 외부 타당성 문제다. m을 늘리면 영상이 줄고, 줄이면 영상이 는다
2  구속 조건은 **SECONDARY 채널**이다 — α=0.0의 분산이 더 크다. 두 endpoint를 모두
   요구하므로 필요 규모는 아래쪽 행(391 / 251 / 176)으로 봐야 한다
3  교차 검증 — 0.06 · m=5 행이 k=35 · 175행 · 달성 0.0596이다. P2의 35영상×5=175와
   정확히 일치한다. **즉 P2 규모는 α=0 채널에서 half-width 0.06급 설계였다**
```

각 목표가 **허용하는 결론**을 같이 본다.

| 목표 | 검출 가능한 효과 크기 감각 | 허용되는 결론 |
|---|---|---|
| 0.04 | AI Hub 관측 Δ(+0.031)·dev 관측 Δ(−0.090) 모두 0 배제 가능 | 교체 근거를 양방향으로 판정할 수 있다 |
| 0.05 | +0.031급은 경계, −0.090급은 판정 가능 | dev 방향 재현은 판정, AI Hub 방향은 애매할 수 있다 |
| 0.06 | +0.031급은 검출 불가에 가깝다 | "3B를 교체할 근거 없음"은 말할 수 있으나 4B 우세 확증은 어렵다 |

**비용 때문에 0.06을 고르는 것이 아니라, 각 목표가 어떤 결론을 허용하는지와 라벨 부담을
...
```
A  half-width / precision rule       0.04 / 0.05 / 0.06 중 (§10-1 표를 보고)
B  k × m 과 총 GT 행 수              A에서 파생. ICC=0이므로 m은 수집 비용 축이다
