...
- **재측정 트리거 (2026-07-11 전 항목 완료):** ① static_threshold 재실측 완료(ablation_plan 2-4-2) — dev 96건 스윕에서 치환 off(thr=0)가 유의 우세(mrr +0.035, CI 0 배제), **static_threshold=0 확정(2026-07-11)**. seg_len(5초 유지)·caption_prompt(P0 유지) ablation도 완료(ablation_plan 1-6, 3-6). ② α 재탐색: `results/alpha_search_dev.json`(dev 96건, 3영상 by_video 분해) — **alpha_star=0.5** 확정(경과: 1차 33건에서는 tiebreak에 의해 1.0=baseline으로 수렴 → dev 확장으로 59·81·96건에서 0.6 안정화 → 오염 캡션 21건 선별 재생성 후 α=0.6이 tie_set에서 탈락하며 0.5로 최종 확정. 상세: docs/archive/평가분석_2026-07-10.md). ③ KURE vs BGE-M3 비교: `archive/ablation/results_bge/alpha_search_dev.json` — KURE-v1이 전 지점(α=1.0/0.0 양끝 포함) 우세 확인, embed_model=KURE-v1 유지 확정.
...
**(4) 동시점 대조군.**
후보와 현행을 **같은 하드웨어·라이브러리·코드 경로에서 함께 생성**해 모델 효과를
비교한다. 목적은 **시간에 따른 라이브러리·코드·생성 경로 차이를 통제**해 후보와
대조군을 공정하게 맞추는 것이다. 2차 캡션 프로브의 "양자화 해제가 유의하게 나쁘다"
(Δ−0.0760, CI [−0.146, −0.0109])는 저장분과 신규 생성분을 섞어 읽은 값이라 그대로
쓸 수 없다.

> **개정 (2026-08-17). 저장된 산출물과 새 산출물의 차이를 별도의 "환경 효과"
> 추정치로 해석하지 마라.** 원래 이 항목은 "생성 환경 효과(GPU·커널·라이브러리)를
> 분리한다"고 썼고, 근거는 2026-08-07 dev 96의 Δ−0.0879 CI[−0.158, −0.023]이었다.
>
> **그 크기의 방향성 페널티는 2026-08-12에 재현되지 않았다.** dev와 독립인
> AI Hub A-half **562질의(MDE ±0.036)**에서 Δ**−0.0046** CI[−0.0267, +0.0174] —
> 0.09는 물론 0.02 넘는 차이도 배제된다. 0.09는 dev 96(MDE ±0.086)의 노이즈였고,
> 08-07과 08-10 두 번 "재현"된 것도 **같은 96건을 두 번 본 것**이라 독립 증거가
> 아니었다.
>
> **문자열 비결정성과 성능 페널티는 다른 현상이다.** 기계를 건너면 캡션 문자열은
> 대부분 달라진다(dev 완전일치 25.6% / AI Hub 97영상·1,164구간 완전일치 23.2%).
> **그러나 그것을 곧 검색 성능 저하로 해석해서는 안 된다.**
>
> **재개정 (2026-08-18) — 같은 조건에서는 결정적이다.** 위에 "같은 기계·같은 코드
> 경로에서도 달라진다(2026-08-17, 655구간 중 8건)"고 적었으나, **같은 commit·같은
> 환경에서 AI Hub 2,328구간을 재생성한 결과 상이 0건·완전일치 1.0**이고 MRR도 소수
> 4자리까지 같았다(`docs/재분석_2x2_2026-08-18.md` §8, arm 2종 각각). 따라서 655구간
> 8건은 greedy 파이프라인의 고유 성질이 아니라 **두 실행 사이의 통제되지 않은 차이**
> 때문이며, 그 차이는 **미확정**이다. 동시점 대조군을 두는 이유는 "생성이 흔들려서"가
> 아니라 **코드 경로·라이브러리가 시간에 따라 달라지기 때문**으로 좁혀진다.
>
> AI Hub 562는 모델 확증에 쓴 **1,086질의의 부분집합**이므로 두 AI Hub 결과를
> 서로 독립된 확증으로 세지 않는다. 근거: `docs/probes/_scratch/aihub_env_check.json`,
> 작업현황 2026-08-12 §2.
>
> **대조군을 두는 관행 자체는 유지한다** — 비용이 없고, 코드 경로·라이브러리가
> 시간에 따라 달라지는 것은 실제로 관측됐다(08-10 sweep 경로 vs 08-14 배포 경로
> 완전일치 20.0%).
