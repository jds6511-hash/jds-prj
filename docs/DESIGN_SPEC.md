...
조치 ④: **적응적 2-pass.** 그리디로 reduce한 뒤 `distinct_ratio`(서로 다른 **서술**의 비율, 인용만 다른 줄은 같은 문장으로 셈)가 `MIN_DISTINCT_RATIO`=0.5 미만이면 그 영상만 `no_repeat_ngram_size=`**12**로 1회 재생성하고 `report.json`의 `reduce_retry`에 1차 비율·1차 원문·재생성 후 비율을 기록한다. 정상 영상은 호출이 1회로 끝나 출력이 보존된다(실측 정상 0.75·0.79 대 루프 0.05·0.13으로 분포가 멀다). 재생성 후에도 0.5 미만이면 `save_report`가 실패한다 — 개수만 세는 검증이 세 번 놓쳤으므로 **다양성 자체를 검증 포인트로** 넣는다. 이를 위해 `llm.make_llm`의 반환 함수가 `**gen_kwargs`를 받아 `mdl.generate`로 넘긴다(이 경로 전용).
