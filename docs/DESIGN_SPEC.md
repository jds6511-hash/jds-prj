...
**(6) M8 서술 공백 — 예시 자리표시자 과교정 (2026-08-06 서버 7B 실측, 수정됨).** 랩실 GPU(RTX 4090 24GB)에서 M8을 처음 실행한 결과, **dev 3영상 전부 리포트가 `- [seg#N]`만으로 채워졌다** — 서술 문장이 하나도 없다(정보량 0). `map_raw_outputs[0]`이 649자/60세그먼트로 map 단계부터 비어 있었으므로 reduce 문제가 아니고, `raw_output` 1,677자는 `max_new_tokens=2048` 상한의 4분의 1이라 잘림도 아니다. 원인은 `_SYSTEM`의 출력 형식이 `- (실제 세그먼트에 근거한 사건 서술) [seg#9999]`처럼 **괄호 자리표시자**였고 거기에 "내용·번호를 절대 복사하지 말 것"을 붙인 것이다 — Qwen2.5-7B가 이를 "내용을 쓰지 말 것"으로 이행했다. **(4)의 3B 예시 복사 사고를 막으려던 방어의 과교정**이다(3B는 예시를 베끼고, 7B는 예시를 안 채운다 — 반대 방향 실패). **더 중요한 것은 검증이 이걸 못 잡았다는 점이다**: `save_report`의 assert가 인용 **범위**만 봐서, 인용이 유효하면 서술이 비어도 통과해 M8이 "완료: 문장 270개"로 성공 보고했다. 수정 3건: ① 예시를 실제 서술 문장으로 채우되 **번호는 9000번대 유지**(복사 방어 불변), ② 규칙 7 "인용만 있고 서술이 없는 줄 금지" 추가, ③ `m8_report.narration()`으로 인용 마커를 걷어낸 서술이 비면 `save_report`가 **문장 단위로 assert 실패**(report.json은 먼저 저장 — 3-5 raw 보존 원칙 유지). 양방향(복사/공백)을 각각 다른 장치로 막는다. dev에서 잡았으므로 **test 접촉 0**이다 — M9는 `split=="test"` 하드코딩이라 M8 출력이 확정되기 전에 돌리면 그 수치를 보고 프롬프트를 고치게 되어 test 오염이 된다.
