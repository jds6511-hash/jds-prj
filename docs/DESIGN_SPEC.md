...
- **어휘 baseline(BM25) 미포함 — 범위 밖(명시):** 본 baseline은 동일 임베딩 파이프라인의 α=1.0(자막 시맨틱 임베딩)이다. 순수 어휘 매칭(BM25)과의 비교는 두지 않았다 — 시맨틱 임베딩 자체가 이미 강한 baseline이며(오타·동의어에 강건), 본 연구 질문은 "자막 채널에 시각 채널을 더한 이득"이지 "어휘 vs 시맨틱"이 아니다. 어휘 baseline 비교는 향후과제로 남긴다. (심사에서 "왜 키워드 검색과 비교 안 했나" 질문 대비.)
