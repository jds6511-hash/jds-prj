...
// sentences[4:11]을 그대로 옮겼다. 문장은 편집하지 않았다. [2026-08-13 피드백]
{
  ...
  s.addText("잘 되는 것 — 문장마다 근거 구간이 붙어 그 시각으로 되돌아갈 수 있다(seg 번호 × 5초). 문장은 고치지 않고 그대로 옮겼다.",
    { x: M + 0.35, y: 5.62, w: 7.0, h: 0.5, fontFace: BFONT, fontSize: 11,
  ...
  s.addText("측정된 문제 — 개선 예정", { x: M + 8.5, y: 1.9, w: 3.1, h: 0.35,
    fontFace: HFONT, fontSize: 17, bold: true, color: INK, margin: 0 });
  const how = [
    ["요약이 아니라 나열", "문장이 구간을 거의 1:1로 훑는다. 문장당 2.1구간, 인용은 100% 시간순. 리포트 4편에서 같은 양상.", VISION],
    ["문장이 똑같이 생겼다", "34%가 “…이 보입니다”로 끝나고 41%가 “남성이”로 시작한다. 존댓말·평어도 섞인다.", VISION],
    ["뒤 2/3가 사라진 원인", "장면 설명 속 한자 2글자에서 모델이 중국어로 바뀌며 생성을 끝냈다. 262구간 중 87까지만 남았다.", SLATE],
  ];
  how.forEach(([t, d, c], i) => {
    const y = 2.4 + i * 1.28;
  ...
  s.addNotes("웹 시연에 M8·M9는 안 나옵니다. 서버 산출물을 편집 없이 옮겼습니다. 오른쪽 세 가지는 리포트 4편을 직접 재서 나온 수치입니다 — 지금 출력은 요약이라기보다 구간 나열에 가깝습니다. 절단 원인은 규명했습니다: 장면 설명 캡션에 섞인 한자 두 글자에서 리포트 모델이 중국어로 전환하며 생성을 끝냈고, 같은 일이 중간 단계와 최종 단계에서 두 번 일어나 뒤 2/3가 사라졌습니다. 생성 상한 문제가 아닙니다(상한의 1/5에서 멈췄습니다). 오염 필터가 한자 3글자 이상만 잡아 2글자가 통과한 것이고, 같은 상태의 캡션이 전체의 7.1%입니다. 채점 수치를 묻거든: 현행 7B 판정자는 정답을 아는 60문항 게이트에서 coverage를 못 갈라 탈락(0.550), 14B는 통과(0.767)했습니다. groundedness는 7B도 통과했습니다(양성 0.817·음성 1.000). 탈락한 계측기의 수치는 싣지 않습니다.");
}
