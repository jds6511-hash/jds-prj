...
8  M8 사건 입도(granularity) 재설계 — **2026-08-28 추가**
   ...
   — 소비 선언: 판정 표본 N=8은 결과 열람으로 소진됐다. 어떤 수정안도 새 8편과
     새 사람 정답으로만 확증할 수 있다
   — 출력 형식: 향후 fresh 데이터에서 재검증할 경우, 사건당 1~2문장과 문장별
     segment citation을 갖는 사건 중심 출력 형식을 사전에 동결한 뒤 평가할 수 있다
     (근거: docs/finalization/M8_HUMAN_REFERENCE_CONTRAST_2026-08-28.md §4).
     **이번 범위에서는 사전등록을 작성하지 않았다**
