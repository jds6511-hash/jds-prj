...
**현재 기준은 r2다** — Scene01 대표 프레임이 인트로 타이틀 오버레이로 핵심 피사체를
가려 A2로 재지정했다(seg0 → seg2, 질의 3개 새로 작성). v1 수치는 동결 문서에 그대로
보존돼 있고 r2와 섞어 합계를 내지 않는다.
근거: `CAPTION_RETRIEVAL_CASESTUDY_AMENDMENT_A2_2026-08-26.md` ·
`runs/casestudy_caption_retrieval/cs_20260826/step6_retrieval_alpha0.json`

```
                        r2 (현재 기준)            v1 (동결 · 참조)
Top-1 적중              3B 2/15   4B 1/15        3B 2/15   4B 2/15
정답 순위가 더 높은 질의    3B 7/15   4B 8/15        3B 4/15   4B 11/15
정답 순위 중위수          3B 52위   4B 20위        3B 31위   4B 10위
평균 캡션 길이            3B 128.5자 4B 76.4자      (동일 — 인덱스 무변경)
```

> **r2에서 Top-1 적중은 3B 2건 · 4B 1건이고, 순위가 더 높았던 질의도 8 대 7로 갈렸다.**
> v1에서 보였던 4B 우위 방향(11/15)은 **장면 하나를 바꾸면 사라진다** — 분모가 15인
> 사례 연구의 수치가 얼마나 불안정한지 그 자체가 관측이다.
> **이 수치는 한 영상 15질의의 설명용 관찰일 뿐 일반 성능 추정치가 아니다.**
> 질의 세트가 동질적이지 않다는 점도 함께 적는다 — r2는 신규 3 + v1 12다.
