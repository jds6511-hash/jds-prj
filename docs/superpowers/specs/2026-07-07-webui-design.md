...
| POST | `/api/search` | `{video_id, query}` → M5 search() Top-3 `[{idx, start, end, score, subtitle, caption}]`, 응답에 정규화 이전 raw 코사인 통계 `raw: {raw_sub_max, raw_sub_mean, raw_cap_max, raw_cap_mean}` 추가(UI 미사용, 무관련 질의 판정 근거용). 매 질의를 `results/search_log.jsonl`에 1줄 append(로깅 실패는 무시) |
...
- 실제 GPU e2e는 gemini_promo(기 인덱싱, 122세그먼트로 가장 작음)로 수동 스모크: 업로드 → 즉시
  done → 검색 → 점프 확인.
...
- 2026-07-09: HIGH-2 반영 — `/api/search` 응답 raw 코사인 통계 필드와 `search_log.jsonl` 로깅 문서화.
