...
def _s1() -> Scenario:
    ...
    asr[10] = "해변으로 내려가 보자."
    asr[11] = ""
    # 캡션은 구간마다 다르게 둔다. 같은 문장을 12번 두면 캡션 채널 전체가 반복
    # 판정에 걸려 기준선 시나리오가 퇴화한다.
    caption = {
        i: "두 여성이 %s 있습니다." % phrase
        for i, phrase in enumerate(
            ["해변에 앉아", "모래를 밟고", "파라솔 아래에서 쉬고", "바다를 보며 서",
             "가방을 열고", "음료를 나눠 마시고", "사진을 찍고", "웃으며 걷고",
             "돗자리를 펴고", "간식을 꺼내", "지도를 펼쳐 보고", "짐을 챙기고"]
        )
    }
