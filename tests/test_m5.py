...
def test_zscore_basic():
    out = zscore(np.array([0.2, 0.4, 0.6]))
...
def test_zscore_degenerate_returns_zeros():
    # std==0 → 0 벡터 (균등 처리, minmax와 동일 계약)
    assert np.allclose(zscore(np.array([0.7, 0.7, 0.7])), 0.0)

def test_combine_order_substitution_after_normalization():
    # 핵심 계약 [v2 8-4]: 치환은 '정규화 이후'. 정규화 전에 치환하면
    # 정적 세그먼트의 원본 s_cap이 분포 통계를 오염시킨다.
    # [2026-07-13 개정] 정규화는 z-score — minmax의 극값 민감성이 dev 실측에서
    # 유의 손실(-0.065 mrr)로 확인됨 (docs/probes/fusion_alternatives_probe.py).
    ...
    z_sub = zscore(s_sub); z_cap = zscore(s_cap).copy()
    z_cap[0] = z_sub[0]                  # 치환은 정규화 이후
    assert np.allclose(out, 0.5 * z_sub + 0.5 * z_cap)
...
def test_alpha_1_is_baseline_pure_subtitle():
    ...
    assert np.allclose(out, zscore(s_sub))          # baseline = α=1.0 특수 경우
...
def test_static_substitution_makes_score_equal_subtitle():
    ...
    for alpha in (0.0, 0.3, 0.7):
        ...
        assert np.allclose(out, zscore(s_sub))
