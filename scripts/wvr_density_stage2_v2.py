...
V2 진단 프롬프트(English-only · event interval)만 쓴다. V1/V1B 프롬프트는
재사용하지 않는다. 보고서를 만들지 않고, 출력을 event evidence로 재사용하지 않는다.
"""
import argparse
import json
import os
...
def run(video: Path, label: str, arm: str, out_dir: Path,
        event: str = events.EVENT_V2) -> dict:
    import torch
    ...
    prompt = diag.SAMPLING_DIAG_PROMPT_V2 % {"window_start": window["start_sec"],
                                             "window_end": window["end_sec"]}
    record = {
        "schema": "wvr_density_stage2_v2", "stage": density.STAGE2,
...
def main(argv=None) -> int:
    parser = argparse.ArgumentParser(description="Stage 2 V2 interval probe")
    parser.add_argument("--video", required=True)
    parser.add_argument("--window", required=True,
                        choices=list(density.SELECTION_LABELS))
    parser.add_argument("--arm", required=True, choices=list(ARMS))
    parser.add_argument("--out-dir", required=True)
    parser.add_argument("--event", default=events.EVENT_V2,
                        choices=[events.EVENT_V2])
    ...
    print("  collapsed=%s unique_sig=%s degenerate=%s valid=%s reasons=%s"
