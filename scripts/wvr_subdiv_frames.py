"""SUBDIVISION_RECOVERY_V1 frame audit 대지 — 두 overlap의 공유 프레임 (2026-09-09).

사전등록: `docs/preregistration/WVR_W00_SUBDIVISION_RECOVERY_V1_2026-09-09.md`

```
추론 없음 · GPU 없음. inference 입력 경로를 바꾸지 않는다
대상      사전등록된 두 overlap R-O1 [12,24) · R-O2 [24,36) — 각 공유 프레임 6장
...
FRAME_DIR_NAME = "frames_subdiv"
AUDIT_MANIFEST = "subdiv_v1_frame_audit.json"
...
def build(video: Path, runs: Path) -> dict:
    ...
    if list(frames[0].size) != [sd.FROZEN_FROM_W00["frame_width"],
                                sd.FROZEN_FROM_W00["frame_height"]]:
    ...
    for frame, stamp, index, decoded in zip(frames, stamps, indices, times):
        ...
        path = frame_dir / ("S%06.1f.png" % stamp)
    ...
    return {
        "schema": "wvr_subdivision_v1_frame_audit", "event": sd.EVENT,
        "prereg": PREREG, "code_git_head": git_head(),
        "recovery_verdict": (summary.get("recovery_gate") or {}).get(
            "recovery_verdict"),
        "video_sha256": fb.sha256_file(video),
        "frame_size": [sd.FROZEN_FROM_W00["frame_width"],
                       sd.FROZEN_FROM_W00["frame_height"]],
        "sampling_fps": sd.SAMPLING_FPS, "decoded_fps": rate,
        "audit_overlaps": list(sd.AUDIT_OVERLAPS),
        "expansion_allowed": sd.FRAME_AUDIT_EXPANSION_ALLOWED,
        ...
        "support_verdict_vocabulary": list(sd.SUPPORT_VERDICTS),
        "sheets": sheets,
    }


def main(argv=None) -> int:
    parser = argparse.ArgumentParser(description="subdivision frame audit 대지")
