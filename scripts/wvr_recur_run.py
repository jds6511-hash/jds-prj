"""RECURSIVE_SUBDIVISION_RECOVERY_V1 실행기 — child 하나당 fresh process 1회.
...
동결    prompt·schema·model·runtime·표집·token cap 전부 C0와 동일
변경    창 길이 12초 · 6초 overlap · 6프레임 (이것만)
순서    raw persist → parse → collapse (정리 전에 원문을 먼저 남긴다)
재시도  없다. 실패한 child는 INVALID provenance로 남긴다
금지    C1·C2 재실행 · 6초로의 자동 재귀 · semantic 판정
```
"""
import argparse
import hashlib
...
def requested_config(child: dict, stamps) -> dict:
    """C0 requested와 창 기하만 다른 설정 표."""
    return {
        "sampling_fps": rc.SAMPLING_FPS, "frames": len(stamps),
...
def observed_artifacts(runs: Path) -> dict:
    """동결 선행 산출물의 현재 해시 (없으면 키를 넣지 않는다)."""
    rows = {}
...
def reference_hashes(runs: Path) -> dict:
    """기존 계보 픽셀 해시 표 — 실패 parent C0 record + frame bank."""
    table, sources, conflicts = {}, [], []

    parent = runs / rc.PARENT_RECORD
    if parent.is_file():
        ...
        sources.append(rc.PARENT_RECORD)
...
def preflight(child_id: str, runs: Path) -> dict:
    """GPU를 쓰기 전에 일정·동결값·선행 산출물 무변경·중복을 확인한다."""
    if child_id not in rc.CHILD_IDS:
        raise RunError("모르는 child: %r" % child_id)
    if rc.RETRY_ALLOWED or rc.RAW_SALVAGE_ALLOWED:
        raise RunError("retry·raw salvage는 금지돼 있다")
    if rc.TOKEN_CAP_INCREASE_APPROVED:
        raise RunError("token cap 상향은 승인되지 않았다")
    if rc.DEEPER_SUBDIVISION_ALLOWED or rc.C1_C2_RERUN_ALLOWED:
        raise RunError("더 깊은 재귀·C1/C2 재실행은 금지돼 있다")
    ...
    if change["inference_config_change"] != "NONE":
        raise RunError("C0 대비 inference 설정이 바뀌었다: %r"
                       % change["differences"])
    ...
    raw_path = runs / ("%s_%s_raw.txt" % (rc.ARTIFACT_TAG, child_id))
    out_path = runs / ("%s_%s.json" % (rc.ARTIFACT_TAG, child_id))
    for path in (raw_path, out_path):
        if path.exists():
            raise RunError("%s 산출물이 이미 있다 — child는 1회다" % path.name)

    unchanged = rc.frozen_artifacts_unchanged(observed_artifacts(runs))
    ...
    return {"child": child, "stamps": stamps, "requested": requested,
            ...
            "frozen_artifacts": unchanged, "reference": reference,
...
def run(video: Path, child_id: str, runs: Path) -> dict:
    ...
    record = {
        "schema": "wvr_recursive_v1", "event": rc.EVENT, "prereg": PREREG,
        "child": child, "parent_child": rc.parent_child(),
        ...
        "retry_allowed": rc.RETRY_ALLOWED,
        "deeper_subdivision_allowed": rc.DEEPER_SUBDIVISION_ALLOWED,
        "semantic_verdict_by_executor": rc.SEMANTIC_VERDICT_BY_EXECUTOR,
        "production_promotion_allowed": rc.PRODUCTION_PROMOTION_ALLOWED,
    ...
    try:
        ...
        record["lineage"] = rc.lineage_report(
        ...
        record["validity"] = rc.child_validity(record, record["video_sha256"])
...
def main(argv=None) -> int:
    parser = argparse.ArgumentParser(
        description="12초 recursive subdivision recovery 실행기")
    parser.add_argument("--video", required=True)
    parser.add_argument("--child", required=True, choices=list(rc.CHILD_IDS))
    ...
    print("  raw=%s obj=%s uniq_raw=%s zero=%s collapsed=%s uniq=%s "
          "degenerate=%s json=%s lineage=%s/%s validity=%s %s"
