"""Run WVR_OVERVIEW_PREVIEW_V2 exactly once and save raw before parsing."""
...
def run(runs: Path, generator_factory=default_generator_factory) -> dict:
    ...
    record = {
        "schema": "wvr_overview_preview_v2_record", "event": ov.EVENT,
...
def main(argv=None) -> int:
    ...
    print("inference_count=%d retry_count=%d raw_saved=true parsed_here=false elapsed=%.3f" %
          (record["inference_count"], record["retry_count"], record["elapsed_sec"]))
    return 0
