...
- **후보 모델이 진 것인지, 파이프라인이 이득을 죽인 것인지 반드시 가른다.**
  ...
  4. **동일 환경 대조군** — 저장된 현행 산출물과 방금 생성한 후보를 직접 비교하지 마라.
     같은 하드웨어·라이브러리에서 현행 설정으로 **새로 생성한 arm**을 넣어야 모델 효과와
     생성 환경 효과가 분리된다.
...
- **확정 인덱스(dev 3편·test 4편)의 캡션을 다른 환경에서 재생성하지 마라** (2026-08-07
  실측). 같은 모델·양자화·프롬프트·그리디인데 노트북(RTX 3060) 생성분과 서버(RTX 4090)
  생성분의 **완전일치가 25.6%뿐**이고, 서버 재생성분은 캡션 단독 dev MRR이 **유의하게
  낮다**(Δ−0.0879 CI [−0.158, −0.023]). 그리디는 환경을 건너면 결정적이지 않다.
  새 영상 추가 시 **그 영상만** 돌린다. 상세 작업현황 2026-08-07 §4-b.
